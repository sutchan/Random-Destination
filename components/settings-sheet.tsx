...
import { 
  X, Plus, MapPin, Trash2, Edit2, Save, Settings2, Heart, HeartOff, Star, History, ChevronRight 
} from "lucide-react"
import { DestinationList } from "@/hooks/use-destination"

import { ChinaRegion } from "@/lib/china-data"
...
export function SettingsSheet({
  ...
  const [activeTab, setActiveTab] = React.useState<'list' | 'browse'>('list')
  const [browsePath, setBrowsePath] = React.useState<string[]>([])

  const getBrowseItems = () => {
    let items = chinaRegions
    for (const path of browsePath) {
    ...
    <Sheet>
      ...
      <SheetContent side="right" className="w-full sm:max-w-md overflow-y-auto" id="settings-sheet">
        ...
        <div className="flex gap-2 mb-4">
          <Button variant={activeTab === 'list' ? 'default' : 'outline'} size="sm" className="flex-1" onClick={() => setActiveTab('list')}>
            {t.listSettings}
          </Button>
          <Button variant={activeTab === 'browse' ? 'default' : 'outline'} size="sm" className="flex-1" onClick={() => setActiveTab('browse')}>
            {t.browseRegions}
          </Button>
        </div>
        ...
        {activeTab === 'list' ? (
          ...
          <div id="browse-content" className="space-y-4">
            <div className="flex items-center gap-2 mb-2">
              <Button variant="ghost" size="sm" onClick={() => setBrowsePath([])}>
                {t.all}
              </Button>
              {browsePath.map((path, i) => (
                <React.Fragment key={i}>
                  <ChevronRight className="w-4 h-4" />
                  <Button variant="ghost" size="sm" onClick={() => setBrowsePath(browsePath.slice(0, i + 1))}>
                    {path}
                  </Button>
                </React.Fragment>
              ))}
